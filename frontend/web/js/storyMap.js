...
var raw_chart_width = screen.width-50;
var all_chart_height = screen.height-250;
...
function SceneNode(chars, start, duration, id, scene_name) {
...
function draw_nodes(scenes, svg, chart_width, chart_height, folder, safe_name) {
    ...
    .call(d3.behavior.drag()
      ...
      .on("dragstart", function(d) { 
      		d3.event.sourceEvent.stopPropagation(); //mod-add
	...
	.attr("transform", null)
        .attr("fill", function(d) { 
        	console.log(d);
        	return d3.rgb(color(d.id)).brighter(0.5).toString(); })
        .style("opacity", 1);
    }
...
function draw_links(links, svg, safe_name) {
	...
	d3.selectAll("[charid=\"" + d.from.comic_name + "_" + d.char_id + "\"]")
		...
		.attr("height", h)
		.attr("font-size", '16px');
		//###### end-mod-add-charname-on-line
    }
    
    function mouseout_cb(d) {
    	d3.selectAll("[class=\"char-name\"]").remove(); //###### mod-add-charname-on-line
		d3.selectAll("[charid=\"" + d.from.comic_name + "_" + d.char_id + "\"]")
		    .style("stroke-opacity", "0.6")
		    .style("stroke-width", link_width);
    }
...
function draw_chart(name, safe_name, folder, tie_breaker, center_sort, collapse, jsonScenes, xmlCharacter) {
	...
	// d3.xml(folder + "/characters.xml", function(x) {
	    ...
	    var xchars = read_chars($.parseXML(xmlCharacter));
	    // cha = jQuery.parseXML(cha);
	    // console.log(x);
	    // console.log(cha);
	    ...
	    var raw_chart_height = all_chart_height;
...
/*	    var svg = d3.select("#chart").append("text")
	    ...
	    */
	    ...
	    draw_nodes(scenes, container, width, height, folder, raw_chart_height, safe_name);
	// }); // d3.xml (read chars)
    // }); // d3.json (read scenes)
}
